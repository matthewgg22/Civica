...
function evt(partial: Record<string, unknown>): Sentry.ErrorEvent {
...
describe('scrubEvent', () => {
  it('redacts SSN, email, phone, applicant_id, and ciphertext from event.extra', () => {
    const result = scrubEvent(evt({
      extra: {
        ssn: '123-45-6789',
  ...
  it('redacts nested PII in event.contexts', () => {
    const result = scrubEvent(evt({
      contexts: {
        form_data: {
          ssn: '123-45-6789',
  ...
  it('strips request body, cookies, and non-content-type headers', () => {
    const result = scrubEvent(evt({
      request: {
