...
// OCR values are gross pay; reported_monthly is from monthly_gross_income answer.
// Delta = (ocr_monthly - reported_monthly) / reported_monthly.
// Flags when absolute delta exceeds 10%.
...
export function compareIncome(
  ...
  const ocr_annual = annualizePayAmount(ocr_pay_amount, ocr_pay_period);
  const ocr_monthly = ocr_annual / 12;
