...
// Zero-dependency table export for the CBO preview tables.
...
//
// Data is passed in as string[][] so it serializes cleanly from server or
// client components that own the tables.

export interface TableExportProps {
  /** Download base name, no extension (e.g. "cbo-pipeline"). */
  filename: string;
  /** Human title printed atop the PDF. */
  title: string;
  /** Column headers. */
  columns: string[];
  /** Row cells as plain strings, already formatted for display. */
  rows: string[][];
  /** Footnote printed under the PDF table. */
  note?: string;
}
    ...
    <div ref={ref} className="relative inline-block">
      ...
      {open && (
        ...
        >
          ...
          >
            CSV <span className="text-graphite">(Excel)</span>
          </button>
          <button
            type="button"
            role="menuitem"
