...
| 2026-06-06 | `/cbo-preview` migrated to institutional (utility-first) mode — see §10 | The CBO-facing demo read "friendly SaaS" (colored risk/verdict/urgency pills, colored dots, colored vertical accent bars, multi-accent progress bars, a card per row). Re-skinned all 5 tabs to a research-desk register: dense tables with hairline row rules, text-only status/verdict/risk, right-aligned tabular numerics, thin sharp monochrome bars, `rounded-[2px]`. Color collapsed to a 2-signal rule (pine = positive, brick = attention) used as **text only**. Warm neutrals + Hanken Grotesk retained, so the surface stays brand-coherent. Scoped to cbo-preview; staff surfaces unchanged for now. |

---

## 10. Institutional mode (`/cbo-preview`)

A utility-first ("investment-bank / research-desk") register applied to the public CBO demo. **Scoped to `/cbo-preview` only** — the staff dashboard keeps the §1–§4 vocabulary until a separate rollout. When extending or rolling this out, follow these rules:

1. **Two signal colors, text-only.** On this surface, color is restricted to **pine** (positive — eligible, on-track, resolved) and **brick** (attention — denied, overdue, high-risk, open flag), and only ever as **text** (or a thin left-rule / thin bar). No accent **fills**, **pills**, or **dots**. Everything else is `ink` / `graphite` / `muted`. Warning/amber/indigo/teal are not used here.
2. **Tables over cards.** Lists, queues, buckets, breakdowns, and stat rows render as `<table>` inside a single `border border-hairline rounded-[2px] bg-surface` container. Header row: `bg-surface-secondary` + `border-b border-hairline`, cells `text-[10px] font-semibold uppercase tracking-wider text-muted`. Body rows: `border-b border-hairline last:border-b-0`, cell padding `py-2`/`py-2.5`. Reserve standalone bordered cards for true top-level grouping only.
3. **Divided metric rows, not card grids.** Stat/KPI groups use one bordered container with `divide-x divide-hairline` (no `gap` + per-card borders). Value `text-[26px]/[28px] tabular-nums text-ink`; label + sub `text-muted` (the sub is NOT pine — it's context, not a positive signal).
4. **Numerics:** every count / % / dollar / date is `tabular-nums`, right-aligned in its column.
5. **Progress bars:** thin (`h-[3px]`), **sharp** (no `rounded-full`), monochrome — `bg-graphite` normally, `bg-ink` for the terminal/strongest step, `bg-brick` only when the row is the attention case. No multi-color bar scales.
6. **Status / verdict / risk are text.** Verdict → "Eligible" (`text-pine`) / "Not eligible" (`text-brick`). Risk → `HIGH` (`text-brick`) / `MED` (`text-ink`) / `LOW` (`text-muted`). Flag status → `Open` (`text-brick`) / `Resolved` (`text-pine`). No `StatusBadge` circle, no `StatusPill`, no `rounded-sm` colored chip.
7. **Radius `rounded-[2px]`** on containers (sharper than the §1 `rounded-[4px]` card default), `0` inside tables. **No vertical accent bars** (`w-2.5 h-5 bg-<accent>`) next to section headers — use a plain `.eyebrow` label + count instead.
8. **Functional controls are exempt.** Toggles (plain-language switch), the flag-sensitivity control, and InfoTip markers stay as-is — they're interactive UI, not decoration.

Reference implementations: `app/cbo-preview/page.tsx` (Overview/Applications/Outreach/Renewals tables + `QueueTableRow`), `components/cbo/EngineHouseholdsPanel.tsx` (determinations table), `components/cbo/QcTab.tsx` (metric row + pillar/flags tables).
