...
enum SNAPBenefitEstimatorStrings {
    ...
    static let householdSizeQuestion = CivicaText(
        ...
        zh: "你家里有多少人?",
        vi: "Hộ gia đình của bạn có bao nhiêu người?"
    )
    ...
    static let elderlyOrDisabledHelper = CivicaText(
        ...
        vi: "Mở thêm các khoản khấu trừ SNAP, gồm cả chi phí nhà ở không giới hạn."
    )
    ...
    static let incomeQuestion = CivicaText(
        ...
        zh: "家庭每月税前收入",
        vi: "Thu nhập hàng tháng của hộ gia đình trước thuế"
    )
    static let incomeHelper = CivicaText(
        ...
        vi: "Cộng tất cả tiền lương, làm việc tự do, trợ cấp thất nghiệp, Social Security và tiền cấp dưỡng con."
    )
    ...
    static let rentQuestion = CivicaText(
        ...
        zh: "每月房租或房贷",
        vi: "Tiền thuê nhà hoặc tiền trả góp nhà hàng tháng"
    )
    static let rentHelper = CivicaText(
        ...
        vi: "Dùng số tiền bạn thực sự trả mỗi tháng — phần của bạn nếu chia sẻ."
    )
    ...
    static func resultContextEligible(stateCode: String?, language: CivicaLanguage) -> String {
        ...
        case .vietnamese:
            return "Đây là số tiền ước tính — \(agency) xem xét toàn bộ đơn của bạn và xác nhận số tiền."
        }
    }
    ...
    static func resultContextMinBenefit(stateCode: String?, language: CivicaLanguage) -> String {
        ...
        case .vietnamese:
            return "Theo luật liên bang, hầu hết hộ 1–2 người được duyệt sẽ nhận ít nhất $24/tháng. \(agency) xác nhận số tiền chính xác của bạn."
        }
    }
    ...
    static let ineligibleHeadline = CivicaText(
        ...
        zh: "超过 SNAP 预估上限",
        vi: "Vượt giới hạn SNAP ước tính"
    )
    static let ineligibleContextGrossOver = CivicaText(
        ...
        vi: "Thu nhập của bạn có vẻ cao hơn mức giới hạn SNAP liên bang cho quy mô hộ gia đình của bạn."
    )
    static let ineligibleContextNetOver = CivicaText(
        ...
        vi: "Sau khi khấu trừ SNAP, thu nhập ròng của bạn vẫn vượt giới hạn SNAP."
    )
    static let ineligibleContextBelowMin = CivicaText(
        ...
        vi: "Với những con số này, công thức cho ra số trợ cấp bằng không. Điều chỉnh tiền thuê nhà hoặc thu nhập có thể thay đổi kết quả."
    )
    static let bbceSoftNote = CivicaText(
        ...
        vi: "Nhiều tiểu bang dùng Tiêu chuẩn Đủ điều kiện Theo Nhóm Diện rộng (BBCE) với giới hạn thu nhập cao hơn — vẫn có thể đáng để nộp đơn."
    )
    ...
    static func applyCTA(stateCode: String?, language: CivicaLanguage) -> String {
        guard SNAPComplianceCopyRegistry.approvedEnglish(for: "estimator_apply_cta") != nil else {
            if language == .vietnamese { return "Nộp đơn xin SNAP" }
    ...
    static let seeTheMathLink = CivicaText(
        ...
        zh: "查看我们是怎么算出来的",
        vi: "Xem chúng tôi đã tính như thế nào"
    )
    ...
    static func entryCardSubtitle(stateCode: String?, language: CivicaLanguage) -> String {
        ...
        guard let approvedEN = SNAPComplianceCopyRegistry.approvedEnglish(for: "estimator_entry_subtitle"),
            ...
            if language == .vietnamese {
                return "Năm câu hỏi. Xem số tiền hàng tháng của bạn trước khi nộp đơn."
            }
    ...
    static let staleRulesBody = CivicaText(
        ...
        vi: "Quy tắc SNAP của Civica được cập nhật lần gần nhất cho năm tài chính hiện tại và có thể chưa phản ánh các thay đổi mới của liên bang hoặc tiểu bang. Hãy dùng số ước tính như một hướng dẫn sơ bộ và xác nhận với cơ quan tiểu bang của bạn."
    )
    ...
    static func federalEstimateNote(stateCode: String?, language: CivicaLanguage) -> String {
        ...
        case .vietnamese:
            return "Ước tính CalFresh. Quyết định chính thức của \(agency) dùng toàn bộ hồ sơ vụ việc của bạn và có thể khác."
        }
    }
    ...
    static let citationFooter = CivicaText(
        ...
        vi: "Số liệu FY26 · COLA của USDA FNS có hiệu lực từ ngày 1 tháng 10 năm 2025"
    )
    ...
    static let disclaimerFooter = CivicaText(
        ...
        vi: "Đây là số ước tính của Civica. Cơ quan tiểu bang của bạn đưa ra quyết định cuối cùng."
    )
