...
enum CivicaNotificationTemplates {
    ...
    /// the backend will read at send time.
    ...
    private static let applicationSubmittedSMS = CivicaNotificationTemplate(
        ...
        body: [
            CivicaText(
                "Your SNAP application is with Massachusetts DTA. Decision usually in about 7 days; the state target is 30. We'll text you when there's news.",
                es: "Tu solicitud de SNAP está con el DTA de Massachusetts. Decisión generalmente en unos 7 días; el objetivo del estado es 30. Te enviaremos un mensaje cuando haya noticias."
            ),
        ],
        buttonLabel: CivicaText("See your status", es: "Ver tu estado"),
    ...
    private static let documentRequestedSMS = CivicaNotificationTemplate(
        ...
        preheader: CivicaText(
            "DTA needs one more thing",
            es: "El DTA necesita una cosa más"
        ),
        body: [
            CivicaText(
                ...
                es: "El DTA necesita una cosa más: un talón de pago reciente. Envía una foto aquí o súbela en la app. Antes del {deadline} para que tu solicitud siga su curso."
            ),
        ],
