...
struct IssueCallCenterView: View {
    ...
    private var exampleCategoryColorMap: [String: Color] {
        ...
        var map: [String: Color] = [
            Self.allExamplesFilterLabel.lowercased(): CivicaColors.pinePrimary,
    ...
    private var repProgressRow: some View {
        ...
        return HStack(spacing: CivicaSpacing.sm) {
            ForEach(Array(progressLabels.enumerated()), id: \.offset) { index, label in
                ...
                Text(label)
                    .font(CivicaTypography.subheadStrong)
                    .foregroundColor((isComplete || isCurrent) ? .white : CivicaColors.ink)
    ...
    private var topTabSelector: some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: CivicaRadius.card, style: .continuous)
                .stroke(CivicaColors.pinePrimary.opacity(0.22), lineWidth: 2)
        )
        .civicaShadow(.card)
        .padding(.horizontal, CivicaSpacing.lg)
        .padding(.top, CivicaSpacing.xs)
    ...
    private func issueCallTopTabButton(_ tab: CivicIssueCallTab) -> some View {
        ...
        return Button {
            ...
            Text(tabNavigationTitle(for: tab))
                ...
                .foregroundColor(isActive ? .white : CivicaColors.ink)
                .lineLimit(1)
    ...
    private func assistantMessageRow(_ message: AssistantChatMessage) -> some View {
        ...
        HStack {
            ...
            .background(
                RoundedRectangle(cornerRadius: CivicaRadius.card, style: .continuous)
                    .fill(isUser ? CivicaColors.pinePrimary : CivicaColors.surfacePrimary)
            )
            .overlay(
                RoundedRectangle(cornerRadius: CivicaRadius.card, style: .continuous)
    ...
    private var assistantChatComposer: some View {
        ...
        .shadow(color: CivicaColors.ink.opacity(0.05), radius: 5, x: 0, y: 2)
        .padding(.horizontal, CivicaSpacing.md)
        .padding(.vertical, CivicaSpacing.sm)
    }
    ...
    private var concernComposerCard: some View {
        VStack(alignment: .leading, spacing: CivicaSpacing.sm) {
                    ...
                    LazyVGrid(columns: [GridItem(.adaptive(minimum: 138), spacing: CivicaSpacing.sm)], spacing: CivicaSpacing.sm) {
                        ForEach(selectableAsks) { ask in
                            Button {
                                ...
                                Text(ask.title)
                                    .font(CivicaTypography.captionStrong)
                                    .foregroundColor(viewModel.selectedAsk == ask ? .white : CivicaColors.ink)
                                    .padding(.horizontal, CivicaSpacing.sm)
                                    .padding(.vertical, CivicaSpacing.sm)
                                    .frame(maxWidth: .infinity)
                                    .background(viewModel.selectedAsk == ask ? CivicaColors.pinePrimary : CivicaColors.surfacePrimary)
                                    .clipShape(RoundedRectangle(cornerRadius: CivicaRadius.control, style: .continuous))
                                    .overlay(
                                        RoundedRectangle(cornerRadius: CivicaRadius.control, style: .continuous)
                                            .stroke(CivicaColors.hairline.opacity(0.8), lineWidth: 1)
            ...
            Button {
                ...
                .frame(maxWidth: .infinity)
                .padding(.vertical, CivicaSpacing.md)
                .background(viewModel.canSubmit ? CivicaColors.pinePrimary : CivicaColors.graphite.opacity(0.45))
                .clipShape(RoundedRectangle(cornerRadius: CivicaRadius.control, style: .continuous))
            }
            .buttonStyle(.plain)
        ...
        VStack(alignment: .leading, spacing: CivicaSpacing.sm) {
            HStack(alignment: .top, spacing: CivicaSpacing.sm) {
                IssueCallRepHeadshotView(official: official)
                    ...
                    .overlay(
                        Circle()
                            .stroke(CivicaColors.hairline.opacity(0.9), lineWidth: 1)
                    )
                    .shadow(color: CivicaColors.ink.opacity(0.06), radius: 2, x: 0, y: 1)
                ...
                Button {
                    ...
                    Group {
                        if shouldShowCallPillOrbit {
                            ...
                            .frame(maxWidth: .infinity)
                            .padding(.vertical, CivicaSpacing.md)
                            .background(primaryCallURL == nil ? CivicaColors.graphite.opacity(0.45) : CivicaColors.pinePrimary)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: CivicaRadius.card, style: .continuous)
                .stroke(
                    ...
                    : (isActive ? CivicaColors.pinePrimary : CivicaColors.hairline.opacity(0.7)),
    ...
    private func trackerIssueProgressBar(completedCalls: Int) -> some View {
        ...
        return GeometryReader { geometry in
            ...
            .overlay(
                Capsule()
                    .stroke(CivicaColors.hairline.opacity(0.5), lineWidth: 0.7)
            )
        }
        .frame(maxWidth: .infinity)
        .frame(height: 10)
        ...
        if CivicaLaunchFeatures.shareActionsEnabled {
            Button {
                ...
                Image(systemName: "square.and.arrow.up")
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: CivicaRadius.control, style: .continuous)
                            .stroke(CivicaColors.pinePrimary.opacity(0.4), lineWidth: 1)
                    )
            }
            .buttonStyle(.plain)
    ...
    private func outcomeHistoryBackground(for outcome: CivicCallOutcome) -> Color {
        ...
        case .followUpRequested:
            return CivicaColors.warningAmber.opacity(0.2)
        case .supportive:
    ...
    private var fallback: some View {
        ZStack {
            ...
            Image(systemName: "person.crop.circle.fill")
                ...
                .scaledToFit()
                .padding(CivicaSpacing.sm)
                .foregroundColor(CivicaColors.graphite.opacity(0.55))
        }
    }
