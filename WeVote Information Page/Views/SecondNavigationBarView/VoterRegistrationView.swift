...
struct VoterRegistrationView: View {
    ...
    private var groupedSections: [RegistrationSection] {
        let sectionSpecs: [(VoterRegistrationCard.Phase, String, String, Color, Color)] = [
            (
                ...
                "",
                CivicaColors.paper,
                CivicaColors.pinePrimary.opacity(0.68)
            ),
            (
                .duringElection,
                ...
                "",
                CivicaColors.paper,
                CivicaColors.warningAmber.opacity(0.78)
            ),
            (
                .postElection,
                postPhaseHeaderText,
    ...
    private func registrationCard(_ card: VoterRegistrationCard) -> some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: CivicaRadius.card, style: .continuous)
                .stroke(CivicaColors.pinePrimary.opacity(0.12), lineWidth: 1)
        )
    ...
    private func checkRegistrationPrimaryButton(_ title: String, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            HStack(spacing: CivicaSpacing.sm) {
                ...
                ZStack {
                    Circle()
                        .fill(CivicaColors.iconOnPrimarySurface)
                    Circle()
                        .stroke(CivicaColors.iconOnPrimaryBorder, lineWidth: 1)
    ...
    private func ballotStatusPrimaryButton(
        ...
        Button(action: action) {
            ...
            .frame(maxWidth: .infinity)
            .padding(.vertical, CivicaSpacing.md)
            .foregroundColor(isEnabled ? CivicaColors.onPrimaryText : CivicaColors.graphite)
            .background(isEnabled ? CivicaColors.pinePrimary : CivicaColors.secondaryButtonFillDisabled)
            .overlay(
                Capsule(style: .continuous)
                    .stroke(
                        isEnabled ? CivicaColors.pinePrimary.opacity(0.25) : CivicaColors.secondaryButtonDisabledBorder,
                        lineWidth: 1
                    )
            )
    ...
    private func registrationReadinessPanel(proxy: ScrollViewProxy) -> some View {
        VStack(alignment: .leading, spacing: CivicaSpacing.sm) {
            ...
            Divider()
                .overlay(CivicaColors.pinePrimary.opacity(0.16))
                .padding(.top, CivicaSpacing.xs)
    ...
    private func readinessLeftField(_ label: String, _ value: String, valueColor: Color = CivicaColors.ink) -> some View {
        VStack(alignment: .leading, spacing: CivicaSpacing.xs) {
            Text(label)
                .font(CivicaTypography.captionStrong)
                .foregroundColor(CivicaColors.graphite)
    ...
    private func registrationGuideStripContent(proxy: ScrollViewProxy) -> some View {
        HStack(spacing: CivicaSpacing.sm) {
            ...
            Text("|")
                .font(CivicaTypography.subheadBold)
                .foregroundColor(CivicaColors.graphite.opacity(0.75))
            registrationGuideStripButton(
    ...
    private func handleCardAction(_ action: VoterRegistrationCard.Action) {
        ...
        case .shareDeadline:
            guard CivicaLaunchFeatures.shareActionsEnabled else { return }
            shareRegistrationDeadline()
        }
    }
