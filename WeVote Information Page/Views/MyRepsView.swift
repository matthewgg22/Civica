...
struct MyRepsView: View {
    ...
    private var matchedSummaryPill: some View {
        HStack(spacing: 6) {
            ...
            ForEach(Array(matchedSegments.enumerated()), id: \.element.id) { index, segment in
                if index > 0 {
                    Text("·")
                        .foregroundColor(VoteNowColors.borderSubtle)
                }
        ...
        .clipShape(Capsule())
        .shadow(color: VoteNowColors.textPrimary.opacity(0.08), radius: 3, x: 0, y: 1)
    }
    ...
    private var matchedSegments: [MatchedSegment] {
        [
            MatchedSegment(id: "federal", title: "Federal", count: matchedFederalCount, countColor: VoteNowColors.ctaBlue),
