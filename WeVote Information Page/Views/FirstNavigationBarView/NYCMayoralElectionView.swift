...
struct NYCMayoralElectionView: View {
            ...
            ForEach(Array(guideMiniNavSections.enumerated()), id: \.element) { index, section in
                ...
                Button {
                    ...
                    Text(section.title)
                        .font(.callout.weight(.semibold))
                        .foregroundColor(
                            selectedGuideMiniNavSection == section ? section.tintColor : VoteNowColors.textSecondary
                        )
                        .lineLimit(1)
    ...
    private func guideMiniNavEarmarker() -> some View {
        Text("|")
            ...
            .foregroundColor(VoteNowColors.textSecondary.opacity(0.72))
            .padding(.horizontal, 0)
    ...
    private func guideCardView(_ card: ElectionGuideInfoCard, index: Int) -> some View {
        ...
        let shadowColor = isFocused ? section.selectedBackgroundColor : VoteNowColors.textPrimary.opacity(0.05)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .stroke(isFocused ? section.selectedBorderColor : VoteNowColors.borderSubtle, lineWidth: 1)
        )
    ...
    private func primaryGuideBodyView(_ context: ElectionGuidePrimaryCardContext) -> some View {
        VStack(alignment: .leading, spacing: CivicaSpacing.sm) {
            ...
            if context.usesTopTwoStyle {
                ...
                (
                    Text("• ").foregroundColor(VoteNowColors.textPrimary)
                    ...
                    Button {
                        ...
                        HStack(alignment: .top, spacing: CivicaSpacing.sm) {
                            ...
                            Image(systemName: isExpanded ? "chevron.up.circle.fill" : "chevron.down.circle.fill")
                                .font(.subheadline)
                                .foregroundColor(VoteNowColors.ctaBlue.opacity(0.9))
                        }
                        .frame(maxWidth: .infinity, alignment: .leading)
                    }
                    .buttonStyle(.plain)
