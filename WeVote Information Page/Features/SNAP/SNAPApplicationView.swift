                ...
                ForEach(visibleChecklistDocuments) { document in
                    ...
                    Button {
                        ...
                        HStack(spacing: 10) {
                            Text(hasDocument ? "●" : "○")
                                .font(.subheadline.weight(.semibold))
                                .foregroundStyle(hasDocument ? VoteNowColors.ctaBlue : VoteNowColors.textSecondary)
                            ...
                            Text(hasDocument ? "Have this" : "Still need / Not sure")
                                ...
                                .overlay(
                                    Capsule(style: .continuous)
                                        .stroke(hasDocument ? VoteNowColors.ctaBlue.opacity(0.28) : VoteNowColors.borderSubtle, lineWidth: 1)
                                )
                        }
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        ForEach(section.rows) { row in
                            ...
                            HStack(alignment: .top, spacing: 10) {
                                Text(row.label)
                                    .font(.footnote.weight(.semibold))
                                    .foregroundStyle(isMissingRequired ? VoteNowColors.ctaRed : VoteNowColors.textSecondary)
                                Spacer(minLength: 8)
                                Text(row.value)
                                    .font(.footnote)
                                    .foregroundStyle(isMissingRequired ? VoteNowColors.ctaRed : VoteNowColors.textSecondary)
...
private struct SNAPFieldBadgeChip: View {
    ...
    private var borderColor: Color {
        switch badge.style {
        case .required:
            return VoteNowColors.ctaBlue.opacity(0.32)
        case .requiredAttention:
            return VoteNowColors.warningAmber.opacity(0.42)
        case .optional:
            return VoteNowColors.borderSubtle
        case .optionalEstimate:
            return VoteNowColors.ctaBlue.opacity(0.2)
        }
    }
}
...
private struct SNAPTernaryChoiceQuestion: View {
    ...
    private func choiceButton(_ choice: SNAPTernaryChoice) -> some View {
        Button {
            ...
            Text(choice.label)
                .font(.subheadline.weight(.semibold))
                .foregroundStyle(value == choice ? VoteNowColors.ctaBlue : VoteNowColors.textPrimary)
                .frame(maxWidth: .infinity, minHeight: 44)
                .background(
                    RoundedRectangle(cornerRadius: 10, style: .continuous)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 10, style: .continuous)
                        .stroke(
                            value == choice ? VoteNowColors.ctaBlue.opacity(0.45) : VoteNowColors.borderSubtle,
                            lineWidth: 1
                        )
                )
        }
...
private struct SNAPYesNoSegmentedQuestion: View {
    ...
    private func yesNoButton(
        ...
        Button(action: action) {
            Text(title)
                .font(.subheadline.weight(.semibold))
                .foregroundStyle(isSelected ? VoteNowColors.ctaBlue : VoteNowColors.textPrimary)
                .frame(maxWidth: .infinity, minHeight: 44)
                .background(
                    RoundedRectangle(cornerRadius: 10, style: .continuous)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 10, style: .continuous)
                        .stroke(
                            isSelected ? VoteNowColors.ctaBlue.opacity(0.45) : VoteNowColors.borderSubtle,
                            lineWidth: 1
                        )
                )
        }
