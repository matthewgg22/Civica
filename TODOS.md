...
## TODO-41 — Error-attribution ledger (event log + Beta-Binomial rollup + flow-DAG)

**What:** Build the reviewed 3-layer error-attribution architecture: an append-only `error_events` table (numerator) joined to the existing `packet_qc_samples` frame (denominator), a trigger-maintained `error_rollup` aggregate with per-slice Beta-Binomial `(α, β)` conjugate weights + empirical-Bayes shrinkage, and a hand-authored `error-flow.yaml` provenance ontology. Extend `scoreErrorRisk()` to v0.4.0 with an optional `sliceWeights` arg (pure; falls back to static `ERROR_WEIGHT`), behind a shadow flag. Full design + sequenced build order in `docs/plans/error-attribution-ledger.md`.
**Why:** The v0.3.0 scorer collapses to one tier/score per packet — structurally unable to answer "which rule overfires in Fresno?" or "intake vs. verification?", the exact questions caseworker mode and the Unrath retention pillar both need. One schema serves all three pillars with an auditable trail (load-bearing under USDA Advanced Automation guidance).
**Pros:** Per-slice attribution with calibrated, traceable weights; honest `event_class` split (prevented vs. confirmed vs. reversed) keeps the rate measuring escaped error, not detector volume; engine stays pure; shadow cutover prevents silent regression to on-screen numbers.
**Cons:** New event-sourcing subsystem + inference layer + DAG; online-incremental rollup (Decision 7) pulls the online-cadence question into v0.4 and needs careful concurrency (no pg_ivm on Supabase — atomic UPSERT counters). Weights are uncalibrated until real QC volume accumulates; per-slice flip is gated on shadow validation.
**Context:** Reviewed via `/plan-eng-review` 2026-05-28 (seven design decisions) + an independent second-opinion pass that found material defects. Two gates before any build (see plan "Post-review amendments"): **A1** — `packet_qc_samples` has no `rule_id`/`county`/`cohort` columns, so the headline per-slice denominator cannot be computed without first denormalizing slice columns + logging evaluated-but-clean rules (hard prerequisite); **A5** — the online/incremental rollup (Decision 7) was challenged on correctness grounds (can't fold `reversed` corrections or maintain EB shrinkage per-row), reviewer recommends nightly matview — operator must confirm/override. A simpler Wilson/Jeffreys-interval view (one migration + one view) may be the right first step. Source finding `docs/findings/2026-05-28-error-attribution-framework.md` (facts corrected same session — cited a non-existent package path + wrong scorer version). Sits atop `packet_qc_samples` (`20260590`), `qc_outcomes` (`20260555`), `v_qc_pillar_coverage`, the shipped `/qc` redesign.
**Effort:** L (human ~1 wk for migrations + emitters + rollup + scorer + tests / CC ~1 d for the build, excluding the post-build shadow-validation window which needs real volume)
...
**Context:** Migration `20260600_packet_outcomes.sql` (the `source` discriminator + `ck_packet_outcomes_self_report_no_dollars` CHECK were built for exactly this). iOS `CountyOutcomeClient.swift` is the self-report side. Eng-review test plan: "PER reads only `source IN (county_authoritative, qc_sample)`."
