...
def test_examples_return_empty_when_fallback_disabled_and_no_db_templates(monkeypatch) -> None:
    monkeypatch.setenv("VOTENOW_ENABLE_BASELINE_EXAMPLE_FALLBACK", "false")
    repo = InMemoryCivicRepository()
    _seed_repo_with_federal_reps(repo, user_id="u-no-fallback")
    service = CivicService(repository=repo)

    examples = service.get_examples("u-no-fallback").to_dict()["examples"]
    assert examples == []
