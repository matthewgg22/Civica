...
// Phase 1: tokens added alongside v1. No component swap yet.
// v1 call-sites keep compiling with deprecation warnings (see shims in CivicaColors.swift).

extension CivicaColors {

    // MARK: Pine — primary CTA, success surfaces, navigation chips
    // AAA on white at 9.01:1. Replaces brickPrimary in every primary-action slot.
    ...
    // NEVER use as text on paper — wheatPrimary on paper is 2.1:1, fails AA at all body sizes.
    // Text on wheat surfaces: ink only.
    ...
    // wheatPop: confident focused/hover accent. Max 1 instance per screen.
    // Supersedes timelineFocusGold in the same functional role.
    public static let wheatPop            = Color.dynamic(light: "#F0CB5A", dark: "#E8C84A")

    // MARK: Brick (accent) — recovery / human moments ONLY
    // Navigator calls, denials, account help, recovery flows.
    // NEVER use for primary CTAs. See brickPrimary shim in CivicaColors.swift.
...
// Canonical tokens live in CivicaColors.*. These aliases let SwiftUI call-sites
// use .foregroundStyle(.civicaPinePrimary) without importing the enum explicitly.
...
// Enables .foregroundStyle(.civicaPinePrimary) syntax at call-sites.
...
// white-on-pine in dark mode is ~2.8:1 — fails WCAG AA.
// Apply this modifier to every primary CTA label instead of .foregroundColor(.white).
