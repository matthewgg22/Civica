...
genuinely-indeterminate profiles, see below) for **20 of those 22**: CA, WA, TX, NY, GA,
MI, IL, FL, MA, NV, AZ, OR, WI, KS, OH, AK, NC, VA. (PA, NJ, and now TN also have oracle
rows authored, per the execution log's PA/NJ/TN entries below, but all three grade
34/0/95 — most of their profiles legitimately SKIP on the null-SUA gap, not a coverage
gap, so none is counted as "clean" here.)
...
NM, OK, RI, SC, SD, UT, VI, VT, WV, WY. (NC, NJ, VA, and TN — the first four "individual
tier" states, §6 — are DONE; see the execution log's NC/NJ/VA/TN entries.)
...
3. Individual tier (~4M+ population): ~~NC~~ (done), ~~NJ~~ (done), ~~VA~~ (done), ~~TN~~ (done), IN, MO, MD, CO, SC, AL, LA, KY, OK
...
- **TN (individual tier, §6 step 3, fourth state after NC/NJ/VA)** — built Tennessee's
  ...
  ***GENUINE STRUCTURAL FINDING, filed as
  ...
  [#831](https://github.com/matthewgg22/Civica/pull/831), awaiting merge go-ahead.
