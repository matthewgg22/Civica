...
|---|--------|---------|----------|---------|---------|-----------|------|-----------------|-------|
| 1 | **Supabase** | Postgres, Auth, Storage, Edge Functions | 🔴 **High** — primary store for applicant_id, packets, encrypted PII, audit log | https://supabase.com/legal/dpa | ❓ Verify | — | — | Re-sign on plan upgrade or T&C update | Required before any prod user. Stores `*_ciphertext` columns, encrypted with `SNAP_FERNET_KEY`. |
