# USDA SNAP QC multi-year — CA payment-error panel (FY2021–FY2023)

`qc_ca_panel.json` — California's error structure by fiscal year, computed from
the USDA SNAP QC public-use microdata (snapqcdata.net) with the same method as the
...
**54.5–64.6%** — so "65/35" is the FY2023 point, not a fixed ratio. Shelter|wages
holds steady ~57–61%.
